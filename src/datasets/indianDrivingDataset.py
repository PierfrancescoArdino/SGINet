...
class IndianDrivingDataset(BaseDataset):
    BASE_DIR = 'indianDrivingDataset'
    NUM_CLASS = 21
    ...
    def __init__(self, root='../datasets', split='train',
        ...
        self.images, self.gts, self.inst_maps, self.inst_data_json =\
            _get_cityscapes_tuple(root, split, self.use_bbox, self.opt.fineTuning, self.opt.how_many)
        if split != 'vis':
...
def _get_cityscapes_tuple(folder, split='train', use_bbox=False, fine_tutning=False, how_many=1500):
